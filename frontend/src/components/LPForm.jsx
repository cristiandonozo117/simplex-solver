...
export function LPForm({ onSolved }) {
  ...
  useEffect(() => {
    if (request) {
      ...
      setCoefs(
        (request.objective?.coefficients || [0, 0]).map((v) => String(v))
      );
      ...
      <form onSubmit={onSubmit}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <label style={{ fontWeight: "600", color: "#181818" }}>
              Variables (n):
            </label>
            <input
              type="number"
              ...
              >
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setCoefs((prev) =>
                      prev.map((v, idx) => (idx === j ? cleaned : v))
                    );
                  }}
                  onBlur={(e) => {
                    ...
                    setCoefs((prev) =>
                      prev.map((v, idx) => (idx === j ? numValue : v))
                    );
                  }}
                  onKeyDown={(e) => {
                    ...
                    if (
                      ...
                      (e.ctrlKey &&
                        [65, 67, 86, 88, 90].indexOf(e.keyCode) !== -1)
                    ) {
                      return;
                    }
                ...
                >
                  <input
                      ...
                      setConstraints((prev) =>
                        prev.map((c, idx) =>
                          idx === i
                            ? {
                                ...c,
                                coefficients: c.coefficients.map((v, jj) =>
                                  jj === j ? cleaned : v
                                ),
                              }
                            : c
                    ...
                    onKeyDown={(e) => {
                      // Allow navigation and editing keys
                      if (
                        [8, 9, 27, 13, 37, 38, 39, 40, 46].indexOf(
                          e.keyCode
                        ) !== -1 ||
                        // Allow: Ctrl+A, Ctrl+C, Ctrl+V, Ctrl+X, Ctrl+Z
                        (e.ctrlKey &&
